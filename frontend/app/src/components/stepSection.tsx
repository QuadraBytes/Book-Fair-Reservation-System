...
const StepSection: React.FC<StepSectionProps> = ({
  ...
  return (
    <section
      className={clsx(
        "mx-auto w-[80%] max-w-6xl px-4",
        "flex flex-col items-center justify-between gap-8 py-16 md:flex-row md:items-center",
        reverse && "md:flex-row-reverse",
        className
      )}
    >
      <div className="relative flex w-full justify-center md:w-1/2 self-center">
        <div className="relative inline-block">
          ...
          <img
            src={image}
            alt={title}
            className="relative z-10 block w-72 rounded-2xl object-cover shadow-lg md:w-96"
          />
        </div>
      </div>
